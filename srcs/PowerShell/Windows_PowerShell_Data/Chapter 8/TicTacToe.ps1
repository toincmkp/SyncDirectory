...
function Get-Permission {
  ...
  while ($startGame -eq "False") {
    ...
    if ($response -eq "Y"){  #The player wants to play a round of Tic-Tac-Toe
      $startGame = "True"    
    }
...
function Validate-Move {
  ...
  if ($move.length -eq 2) {  #Valid moves consist of 2 characters
    if ($move -match "[A-C][1-3]") {  #Regular expression test that looks for 
      $result = "Valid"               #an instance of A, B, or C and an instance
    }                                 #of 1, 2, or 3.
    else {
      $result = "Invalid"  #The move is invalid if it is not A1, A2, A3, B1, B2,
    }                      #B3, C1, C2, or C3
  }
  else {
  ...
  #Move is invalid if it has already been assigned to a player during a previous
  #turn
...
Get-Permission  #Call function that asks the players for permission to start a 
                #new round of play

While ($Terminate -ne "True") {  #Loop until the player decides to quit

  while ($playGame -eq "True") {  #This loop controls the logic required to play
                                  #a round of Tic-Tac-Toe

    $move = Display-Board  #Call function that display the game board and
    ...
    if ($validMove -eq "Valid") {  #Process valid moves
      ...
      #Assign the appropriate game board square to the player who selected it
  ...
  #This next set of statements only runs when the current round of play has ended

  $response = "False"  #Set default value to ensure the loop executes
